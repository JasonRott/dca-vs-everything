...
1. **資訊崩塌**：完美區間資訊下網格年化可達 +320%（理論上限），但資訊退化到
   ...
   而位置（方向）不可預測。
2. **DGT（arXiv:2506.11921）不可複製**：等額現金流下與 DCA 逐點重合
   （最密配置淨值比恆為 0.995 = 付 0.5% 磨損複製無腦買入）；
   官方引擎 + 無限注資假設下，15 組參數全部輸給現金流配對買入持有；
   論文宣稱的 60–70% IRR 以其自身引擎無法重現（最高 27.4%）。
...
| 報告 | 內容 |
|---|---|
| [比較分析報告](reports/比較分析報告.md) | 全策略統一比較、風險-報酬效率前緣、DGT 三層終審 |
...
| 12 | DGT 官方引擎終審（1m 資料） | 15/15 輸現金流配對 B&H；頭條數字不可重現 |
...
- **真實市場定價**：選擇權以 Deribit DVOL（實際隱含波動率指數）+ Black-Scholes 定價，
  附 IV 倍數敏感度。
- **含時間戳現金流的 XIRR**：揭穿「分批注資當期初投入」的 IRR 美化（DGT 批判核心）。
- **對照組設計**：現金流配對買入持有，把「資金行程表的報酬」與「策略的報酬」分離。
- **逐筆引擎 + 官方程式碼交叉驗證**：DGT 批判同時用機制等價重現與其官方引擎雙路徑。
...
python src/dgt_faithful.py         # exp12：DGT 終審（需先抓 1m 資料，較久）
...
**所有結論以「正漂移樣本」為前提**——在漂移平/負的資產上，防禦型結論將反轉。
